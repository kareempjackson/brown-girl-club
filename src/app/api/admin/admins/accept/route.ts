...
import { createAdminToken, verifyAdminInviteToken } from '@/lib/admin-auth';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const invite = verifyAdminInviteToken(token);
    if (!invite) {
      return NextResponse.redirect(new URL('/admin/login?error=invalid_invite', req.nextUrl));
    }

    const adminJwt = createAdminToken(invite.email, 'admin', COOKIE_MAX_AGE);
